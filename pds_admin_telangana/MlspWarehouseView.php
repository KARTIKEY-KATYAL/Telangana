...
<style>
    ...
    /* Optional: Font size for table header */
                ...
                <!-- START BREADCRUMB -->
                <ul class="breadcrumb">
                    <li><a href="MlspWarehouse.php">Home</a></li>
                    <li class="active">MLSP Warehouse View</li>
                </ul>
                <!-- END BREADCRUMB -->
    ...
    <!-- START SCRIPTS -->
		...
		document.getElementById('downloadCSV').addEventListener('click', async function() {
			try {
				...
				const csvResponse = await fetch('api/DownloadOptimalDataMlspWarehouse.php?format=csv&tableName='+tableName);
				const csvBlob = await csvResponse.blob();
				downloadFile(csvBlob, 'Telangana_MlspWarehouse_' + getDateString() + '.csv');
			} catch (error) {
		...
		document.getElementById('downloadXLSX').addEventListener('click', async function() {
			try {
				var tableName = '<?php echo $tablename ?>';
				const excelResponse = await fetch('api/DownloadOptimalDataMlspWarehouse.php?format=xlsx&tableName='+tableName);
				const excelBlob = await excelResponse.blob();
				downloadFile(excelBlob, 'Telangana_MlspWarehouse_' + getDateString() + '.xlsx');
			} catch (error) {
		...
		document.getElementById('downloadPDF').addEventListener('click', async function() {
			try {
				var tableName = '<?php echo $tablename ?>';	
				const pdfResponse = await fetch('api/DownloadOptimalDataMlspWarehouse.php?format=pdf&tableName='+tableName);
				const pdfBlob = await pdfResponse.blob();

				const url = window.URL.createObjectURL(pdfBlob);
				const link = document.createElement('a');
				link.href = url;
				link.download = 'Telangana_MlspWarehouse_' + getDateString() + '.pdf';
				link.click();
