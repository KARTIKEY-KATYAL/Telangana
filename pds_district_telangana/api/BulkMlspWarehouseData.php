	...
	if ($_FILES["file"]["size"] > 0) {
		...
		while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
			if($i > 0){
				...
				$MlspWarehouse = new MlspWarehouse;
				$uniqueid = uniqid("MW_");
				$MlspWarehouse->setUniqueid(substr($uniqueid, 0, 15));
				$MlspWarehouse->setDistrict(ucwords(strtolower($column[$district])));
				$MlspWarehouse->setLatitude($column[$latitude]);
				$MlspWarehouse->setLongitude($column[$longitude]);
				$MlspWarehouse->setName($column[$name]);
				$MlspWarehouse->setId($column[$id]);
				$MlspWarehouse->setType($column[$type]);
				$MlspWarehouse->setStorage($column[$storage]);
				$MlspWarehouse->setWarehousetype($column[$warehousetype]);
				$MlspWarehouse->setActive($column[$active]);
				while(true){
					$query_check = $MlspWarehouse->check($MlspWarehouse);
				...
				$query_insert_check = $MlspWarehouse->checkInsert($MlspWarehouse);
				$query_insert_result = mysqli_query($con, $query_insert_check);
				$numrows_insert = mysqli_num_rows($query_insert_result);
				if($numrows_insert == 0){
					$query_add = $MlspWarehouse->insert($MlspWarehouse);
					mysqli_query($con, $query_add);
				}
				else{
					echo "Error : Warehouse with id ".$MlspWarehouse->getId()." Already Exist</br>";
					$redirect = 2;
				}
			}
			else{
